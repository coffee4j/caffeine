 ...
 * input at all. This means (1, 2, -) is very likely the combination causing the failure of (1, 2, 3). Of course, a
 * combination with fewer than t values can also be failure inducing. Therefore, BEN reduces all t-value- combinations
 * at the end. This is done by looking if all possible containing combinations are suspicious. For example, if each
 * parameter has 2 values and (0, 1) and (0, 2) are suspicious, (0, -) is therefore also suspicious since there is no
 * non-suspicious combination containing (0, -).
 * <p>
 * Important Information:
 * <ul>
 *     <li>Will not find any failure-inducing combination involving more than t parameters. If you expect such failure-inducing combinations, your t is already chosen wrong
 *     <li>Can generate many additional test inputs if needed. The number of generated test inputs per iteration is configurable
 *     <li>Orders failure-inducing combinations by probability
 *     <li>Considers constraints as an addition to the original algorithm
 *     <li>Not very efficient if failure-inducing combination is smaller than t values
 * </ul>
 */
public class Ben extends SuspiciousCombinationAlgorithm {
     ...
     *
     * @param configuration               for knowing which combinations can be failure-inducing/which test inputs can
     *                                    be generated. Must not be {@code null}
     * @param numberOfCombinationsPerStep how many test inputs should be generated per step. Must be positive
     * @param maxGenerationAttempts       after how many attempts the algorithm should give up on generating a
     *                                    previously untested test inputs containing a given failure-inducing
    ...
    /**
     * Can be used as a convenience method to describe that BEN should be used as a {@link
     * FaultCharacterizationAlgorithmFactory}.
     *
     * @return a factory using ben as a fault characterization algorithm. Each instance of BEN is configured to create
     * 10 new test inputs per iteration and uses 50 generation attempts before considering a combination to be
     * failure-inducing
     */
     ...
     * @return a factory using the constructor ({@link Ben#Ben(FaultCharacterizationConfiguration, int, int)}) to create
     * new {@link FaultCharacterizationAlgorithm} instances
     * @throws IllegalArgumentException if one of the integers is not positive
     */
    ...
    private IntList[] computeParameterValueRanking(Object2DoubleMap<Component> componentSuspiciousnessMap) {
        ...
        for (int parameter = 0; parameter < numberOfParameter; parameter++) {
            ...
            parameterValueRanking[parameter] = new IntArrayList(IntStream.range(0, getModel().getSizeOfParameter(parameter))
                    .boxed().sorted(Comparator.comparingDouble(value -> componentSuspiciousnessMap.getDouble(new Component(finalParameter, value))))
