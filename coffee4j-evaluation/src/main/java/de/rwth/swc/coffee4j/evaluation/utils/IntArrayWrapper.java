...
/**
 * A wrapper class for integer arrays so that they can be used inside collections. The default {@link
 * Object#equals(Object)} and {@link Object#hashCode()} implementation depend only on the storage location of an array
 * and thus comparing to arrays via equals does not make sense and the hash code does not help either. This class just
 * wraps the array and uses {@link Arrays#equals(int[], int[])} and {@link Arrays#hashCode(int[])}.
 */
public final class IntArrayWrapper {
    ...
    /**
     * Convenience method for wrapping all int[] in a collection into a set. As the wrappers contain sensible hashcode
     * and equals functions, no duplicates are present in the set.
     *
     * @param arrays the arrays to wrap
